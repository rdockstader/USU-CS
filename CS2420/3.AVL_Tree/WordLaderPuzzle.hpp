...
public:
	...
	std::string playAStar(std::string fromWord, std::string toWord);
...
private:
	//queue variables
	int enqueuedAmount = 0;
	int dequeuedAmount = 0;
	...
	std::string playPrivate(std::string fromWord, std::string toWord, bool isAStar);
	...
	std::string outputPath(std::shared_ptr<Node<std::string>> foundNode, std::string methodType);
	//function to change word and check against dictionary
	void findAndInsertRealWords(std::vector<char> parsedWord, std::shared_ptr<Node<std::string>> curNode, LinkedList<std::string>& list, AvlTree<Node<std::string>>& avlList, bool isAStar);

...
{
	createVectors();
	return playPrivate(fromWord, toWord);
}

//private play function
std::string wordLadderGame::playPrivate(std::string fromWord, std::string toWord)
{
	...
	else
	{
		...
		while (curNode != nullptr)
		{
			...
			findAndInsertRealWords(parseString(curNode->data), curNode, wordsList);
...
void wordLadderGame::findAndInsertRealWords(std::vector<char> parsedWord, std::shared_ptr<Node<std::string>> curNode, LinkedList<std::string>& list)
{
...
std::string wordLadderGame::outputPath(std::shared_ptr<Node<std::string>> foundNode)
{
